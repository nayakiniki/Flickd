...
NEXT_PUBLIC_APP_NAME=Flickd Smart Tagging
...
1. **New Tag Categories**: Modify the analysis logic in \`app/api/analyze/route.ts\`
2. **UI Components**: Add new components in the \`components/\` directory
3. **Styling**: Update Tailwind classes or add custom CSS
4. **Types**: Define new types in \`types/analysis.ts\`
...
1. **Build Errors**: Run \`npm run build\` locally to check for issues
2. **API Errors**: Check \`/api/debug\` endpoint for runtime information
3. **Upload Issues**: Ensure images are under 10MB and valid formats
4. **Deployment Issues**: Check Vercel function logs in dashboard
